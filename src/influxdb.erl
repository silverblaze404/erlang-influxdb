...
-export([query/2, query/3, query/4, write/2, write/3, write_async/2, write_async/3]).
...
url_query(Config, Options) ->
    maps:fold(fun (precision, Value, Acc) ->
       ...
       Url = influxdb_uri:encode(#{scheme => "http",
                                   ...
                                   query =>
                                       maps:fold(fun (precision, Value, Acc) ->
                                                 ...
                                                 Options)}),
       FinalBody =
           lists:foldl(fun({_Config, EncodedMeasurements, _Options}, BodyAcc) ->
